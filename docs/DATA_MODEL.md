...
| `channel` | VARCHAR | `email` \| `sms` \| `whatsapp` \| `slack` \| `webhook` |
| `status` | VARCHAR | `QUEUED` \| `PROCESSING` \| `SENT` \| `FAILED` \| `DEAD` |
| `recipient` | VARCHAR(512) | Destination: email, E.164 phone, or Slack/webhook URL |
| `subject` | VARCHAR | Nullable (unused for SMS/WhatsApp) |
